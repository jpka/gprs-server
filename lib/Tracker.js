...
function Tracker(connection, options) {
  ...
  this.connection.on('data', function(buffer) {
    ...
    // Emit incoming message event
    tracker.emit('packet.in.afterImei', message);
  ...
  this.disconnectionTest = function disconnectionTest() {
  ...
  if (this.config.timeout > 0) {
    this.connection.setTimeout(this.config.timeout, function() {
      console.log("[SIMULATOR] timed out");
      tracker.disconnectionTest();
      // tracker.disconnect();
    });
  }

  connection.setKeepAlive(true, 7 * 60 * 1000);
}
 ...
 * @param {function} callback
 */
Tracker.prototype.requestFirmwareVersion = function(callback) {
  var tracker = this;
  this.request(new Message({
    command: Message.commands.SERVER_COMMAND,
    data: ["F"]
    ...
    var startOfResponse = `${Message.commands.SERVER_COMMAND}$${tracker.trackerId}`;
    var responseIndex = response.raw.indexOf(startOfResponse);
    if (responseIndex > -1) {
      var startOfData = responseIndex + startOfResponse.length + 1;
      callback(null, response.raw.substring(startOfData, startOfData + 6));
    } else {
      callback(new Error("Tracker responded incorrectly: " + response.raw));
    }
  }, true);
