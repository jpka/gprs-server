...
function Tracker(connection, options) {
  ...
  this.connection.on('data', function(buffer) {
    ...
    if (!tracker.trackerId && message.trackerId) {
      ...
      tracker.pin = (tracker.trackerId % 575) * (tracker.trackerId % 306);
    }
    ...
    if (message.command === Message.commands.LOGIN) {
      ...
      // Process queue or set state as ready after initial heartbeat
    ...
    if (message.command === Message.commands.ALARM) {
      var parsed = Message.parseData(Message.commands.ALARM, new Buffer(message.data));
 ...
 * @param {function} callback
 */
Tracker.prototype.requestReport = function(type, callback) {
  this.request(new Message({
    command: Message.commands.REQUEST_REPORT, 
    data: type === "gps" ? "G" : "L"
    ...
    callback(null, Message.parseData(Message.commands.REQUEST_REPORT, new Buffer(response.data)));
  }); 
};
 ...
 * @param {function} callback
 */
Tracker.prototype.setSilentAlarm = function(enable, callback) {
  this.request(new Message({
    command: Message.commands.SET_SILENT_ALARM, 
    data: [enable ? "S1" : "S0"]
  }), function(error, response) {
    if (error) {
      callback(error);
    ...
    if (response.raw === Message.commands.SET_SILENT_ALARM + "$" + response.trackerId + "#" + (enable ? "S1" : "S0")) {
      callback(null);
    } else {
