...
Message.parseData = function(code, data) {
  ...
  function parseCoords(data) {
    ...
    if (data.length > 12) {
      systemState = hex2bin(data[12]);
    }

    return {
      date             : (date && isNaN(date.getTime())) ? null: date,
      latitude         : parseFloatOrNull(data[3]),
      longitude        : parseFloatOrNull(data[4])
    }
  }
  ...
  switch (code) {
    ...
    case Message.commands.REPORT:
      ...
      data = Object.assign(parseCoords(data), {
        speed            : parseFloatOrNull(data[5]),
        heading          : parseFloatOrNull(data[6]),
        altitude         : parseFloatOrNull(data[7]),
        internalBattery  : (parseInt(data[8]) / 10) || null,
        vehicleBattery   : (parseInt(data[9]) / 10) || null,
        gsmSignal        : parseFloatOrNull(data[10]),
        gpsSatellites    : parseFloatOrNull(data[11]),
        ...
        systemArmed      : bin2bool(systemState[5]),
