...
Message.commands = {
  ...
  FIRMWARE_DELETE                 : "OD",
  FIRMWARE_UPDATE                 : "OU",
  FIRMWARE_START_LINEAR_ADDRESS   : "OS",
  FIRMWARE_RECORD_READ            : "OR",
  FIRMWARE_RECORD_WRITE           : "OW"
...
Message.parseData = function(code, data) {
  ...
  function systemStateAt(position) {
    if (position <= systemState.length) {
      return systemState[systemState.length - position];
    } else {
      return null;
    }
  }
   ...
   * @return {object}
   */
  function parseCoords(data, skipDate) {
    var result = {
      latitude:  sxg2dec(data[skipDate ? 2 : 3], 2, data[skipDate ? 3 : 4] === "S"),
      longitude: sxg2dec(data[skipDate ? 4 : 5], 3, data[skipDate ? 5 : 6] === "W"),
      date: null
    };
    ...
    if (data.length > 1 && data[1].length > 0) {
      if (!skipDate) {
        result.date = new Date(
          Date.UTC('20' + data[1].substring(4, 6),
        ...
        result.date = new Date();
        result.date.setHours(data[1].substring(0, 2));
        result.date.setMinutes(data[1].substring(2, 4));
        result.date.setSeconds(data[1].substring(4, 6));
      }
      result.date = isNaN(result.date.getTime()) ? null: result.date;
    }
  ...
  switch (code) {
    ...
    case Message.commands.REPORT:
      ...
      data = Object.assign(parseCoords(data), {
        ...
        gsmSignal        : parseFloatOrNull(data[12]) === 33 ? 100 : parseFloatOrNull(data[12]) * 3,
        gpsSatellites    : parseFloatOrNull(data[13]),
  
        // -- System state --
        vehicleContact   : bin2bool(systemStateAt(1)),
        accelerometer    : bin2bool(systemStateAt(2)),
        energySavings    : bin2bool(systemStateAt(3)),
        externalFeed     : bin2bool(systemStateAt(4)),
        gpsState         : bin2bool(systemStateAt(5)),
        alarmArmed       : bin2bool(systemStateAt(6)),
        engineCut        : bin2bool(systemStateAt(7)),
        siren            : bin2bool(systemStateAt(8)),
        ain2             : bin2bool(systemStateAt(9)),
        din1             : bin2bool(systemStateAt(10)),
        din2             : bin2bool(systemStateAt(11)),
        din3             : bin2bool(systemStateAt(12)),
        din4             : bin2bool(systemStateAt(13))
        // -- ** --
      });
      break;
    case Message.commands.ALARM:
      data = {
        type: data[6],
        data: parseCoords(data, true)
      };
      break;
    case Message.commands.FIRMWARE_RECORD_READ:
...
Message.prototype.toBuffer = function(options) {
  ...
  result = result.concat(this.data.join("#")).join("");

  return new Buffer(result, "ascii") + "\r";
};
