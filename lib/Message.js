...
Message.createFromBuffer = function(buffer) {
  ...
  if (data.length < 3 || data[0] !== "PA" || data[data.length - 1] !== "AP") {
    return {
      invalid: true,
      raw: str
    };
  ...
  if (data[2][0] === Message.commands.REPORT && data[2][1] === "#") {
    command = Message.commands.REPORT;
  } else {
    command = data[data.length - 2];
  }
...
Message.parseData = function(code, data) {
  ...
  function parseCoords(data) {
    ...
    // Retrieve date & time
    var date = new Date(Date.UTC('20' + data[1].substring(0, 2),
...
Message.prototype.toBuffer = function() {
  ...
  if (this.command === Message.commands.SET_REPORT_TIME_INTERVAL) {
    result = ["PA", "2", this.command, this.data, "AP"];
  } else {
    result = ["PA", this.trackerId, this.command, "AP"];
  }
